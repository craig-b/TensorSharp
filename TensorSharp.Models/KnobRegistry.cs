...
namespace TensorSharp.Models
{
    ...
    /// <summary>How a boolean knob's env var is interpreted. Every bool knob
    /// accepts the same canonical tokens, case-insensitive — <c>1</c>/
    /// <c>true</c>/<c>yes</c>/<c>on</c> and <c>0</c>/<c>false</c>/<c>no</c>/
    /// <c>off</c> — so the dialect only states the default and the var's
    /// polarity. Unset or empty → the default; an unrecognized value warns
    /// once and uses the default. <see cref="KnobResolver"/> IS the knob's
    /// env read, so a misstated dialect changes runtime behaviour, not just
    /// docs.</summary>
    public enum BoolDialect
    {
        /// <summary>Unset → on.</summary>
        DefaultOn,

        /// <summary>Unset → off (opt-in knob).</summary>
        DefaultOff,

        /// <summary>DISABLE_*-style opt-out var: a true token disables the
        /// feature. The property is positive-sense; unset → on.</summary>
        InvertedDisable,
    }
    ...
    public static class KnobRegistry
    {
        ...
        {
            // ---- ModelOptions ---------------------------------------------
            B(nameof(ModelOptions.MlxMlockGguf), KnobScope.Model, "TS_MLX_MLOCK_GGUF", BoolDialect.DefaultOn,
                "mlock(2) the GGUF mapping so weights stay resident (MLX)."),
            B(nameof(ModelOptions.GgmlF32Resident), KnobScope.Model, "TS_GGML_F32_RESIDENT", BoolDialect.DefaultOn,
            ...
            B(nameof(ModelOptions.GgmlTpFusedMatmul), KnobScope.Model, "TS_GGML_TP_FUSED_MATMUL", BoolDialect.DefaultOff,
                "Submit both TP ranks' linears from one thread."),
            B(nameof(ModelOptions.MlxDeviceKvCopy), KnobScope.Model, "TS_MLX_DEVICE_KV_COPY", BoolDialect.DefaultOn,
                "On-device MLX KV scatter."),
            B(nameof(ModelOptions.MlxFusedKvWrite), KnobScope.Model, "TS_MLX_FUSED_KV_WRITE", BoolDialect.DefaultOn,
                "Single multi-dim slice_update per KV block (MLX)."),
            B(nameof(ModelOptions.PrefillWarmup), KnobScope.Model, "TS_PREFILL_WARMUP", BoolDialect.DefaultOn,
            ...
            B(nameof(ModelOptions.MlxKernelWarmup), KnobScope.Model, "TS_MLX_KERNEL_WARMUP", BoolDialect.DefaultOff,
                "Force MLX kernel warmup despite large resident quantized weights."),
            B(nameof(ModelOptions.EncoderYield), KnobScope.Model, "TS_ENCODER_YIELD", BoolDialect.DefaultOn,
            ...
            B(nameof(ModelOptions.CudaMoeOnDevice), KnobScope.Model, "TS_CUDA_MOE_ONDEVICE", BoolDialect.DefaultOn,
                "On-device direct-CUDA MoE decode."),
            B(nameof(ModelOptions.CudaMoePrefillOnDevice), KnobScope.Model, "TS_CUDA_MOE_PREFILL_ONDEVICE", BoolDialect.DefaultOff,
                "On-device batched MoE prefill."),
            B(nameof(ModelOptions.CudaMoePrefillGrouped), KnobScope.Model, "TS_CUDA_MOE_PREFILL_GROUPED", BoolDialect.DefaultOn,
                "Grouped gather/scatter CUDA MoE prefill."),

            // ---- Qwen35Options --------------------------------------------
            B(nameof(Qwen35Options.FullDecode), KnobScope.Qwen35, "TS_QWEN35_FULL_DECODE", BoolDialect.DefaultOn,
                "Whole-model fused decode graph (CUDA/Vulkan/Metal)."),
            B(nameof(Qwen35Options.MetalTokenInput), KnobScope.Qwen35, "TS_QWEN35_METAL_TOKEN_INPUT", BoolDialect.DefaultOn,
                "Metal token-id input vs legacy host-dequantized embedding."),
            B(nameof(Qwen35Options.FusedVerify), KnobScope.Qwen35, "TS_QWEN35_FUSED_VERIFY", BoolDialect.DefaultOn,
                "Fused multi-token MTP-verify trunk."),
            B(nameof(Qwen35Options.VerifyResident), KnobScope.Qwen35, "TS_QWEN35_VERIFY_RESIDENT", BoolDialect.DefaultOff,
                "Device-resident GDN verify state."),
            B(nameof(Qwen35Options.MtpFusedDraft), KnobScope.Qwen35, "TS_MTP_FUSED_DRAFT", BoolDialect.DefaultOn,
                "Fused MTP draft/catch-up block."),
            B(nameof(Qwen35Options.PrefillVerify), KnobScope.Qwen35, "TS_QWEN35_PREFILL_VERIFY", BoolDialect.DefaultOn,
            ...
            B(nameof(Qwen35Options.Batched), KnobScope.Qwen35, "TS_QWEN35_BATCHED", BoolDialect.DefaultOn,
            ...
            B(nameof(Qwen35Options.BatchedFused), KnobScope.Qwen35, "TS_QWEN35_BATCHED_FUSED", BoolDialect.DefaultOn,
                "Fused batched decode graph."),
            B(nameof(Qwen35Options.BfdNoMirror), KnobScope.Qwen35, "TS_QWEN35_BFD_NOMIRROR", BoolDialect.DefaultOff,
                "Skip host mirror in batched fused decode."),
            B(nameof(Qwen35Options.BatchedGdnNative), KnobScope.Qwen35, "TS_QWEN35_BATCHED_GDN_NATIVE", BoolDialect.DefaultOff,
                "Native batched GDN kernels."),
            B(nameof(Qwen35Options.Migrate), KnobScope.Qwen35, "TS_QWEN35_MIGRATE", BoolDialect.DefaultOn,
                "Sequence migration between batched and per-seq paths."),
            B(nameof(Qwen35Options.MlxTensorPagedAttn), KnobScope.Qwen35, "TS_QWEN35_MLX_TENSOR_PAGED_ATTN", BoolDialect.DefaultOff,
                "MLX tensor-level paged attention."),
            B(nameof(Qwen35Options.FusedRecPrefill), KnobScope.Qwen35, "TS_QWEN35_FUSED_REC_PREFILL", BoolDialect.DefaultOn,
                "Fused recurrent prefill."),
            B(nameof(Qwen35Options.CudaGdnNative), KnobScope.Qwen35, "TS_CUDA_QWEN35_GDN_NATIVE", BoolDialect.DefaultOn,
                "Native CUDA GDN kernels."),
            B(nameof(Qwen35Options.MlxGdnPackedKernels), KnobScope.Qwen35, "TS_MLX_QWEN35_GDN_PACKED_KERNELS", BoolDialect.DefaultOn,
                "Packed MLX GDN kernels."),
            B(nameof(Qwen35Options.MetalGdnInplaceState), KnobScope.Qwen35, "TS_QWEN35_METAL_GDN_INPLACE_STATE", BoolDialect.DefaultOn,
            ...
            B(nameof(Qwen35Options.GdnVerifyChunked), KnobScope.Qwen35, "GDN_VERIFY_CHUNKED", BoolDialect.DefaultOff,
                "Chunked GDN verify (CI/debug)."),
            B(nameof(Qwen35Options.FusedQkNormRope), KnobScope.Qwen35, "TS_FUSED_QKNORM_ROPE", BoolDialect.DefaultOn,
                "Fused QK-norm + RoPE."),
            ...
            B(nameof(Qwen35Options.MlxChunkedVectorPrefill), KnobScope.Qwen35, "TS_MLX_CHUNKED_VECTOR_PREFILL", BoolDialect.DefaultOff,
                "Chunked MLX vector prefill."),
            B(nameof(Qwen35Options.MlxGpuDeinterleave), KnobScope.Qwen35, "TS_MLX_QWEN_GPU_DEINTERLEAVE", BoolDialect.DefaultOff,
                "GPU strided-view Q/gate deinterleave."),
            B(nameof(Qwen35Options.MropeNative), KnobScope.Qwen35, "TS_QWEN35_MROPE_NATIVE", BoolDialect.DefaultOn,
            ...
            B(nameof(Qwen35Options.StackedMoe), KnobScope.Qwen35, "TS_QWEN35_STACKED_MOE", BoolDialect.DefaultOn,
                "Stacked-experts MoE weights."),
            B(nameof(Qwen35Options.MlxBatchedMoeDecode), KnobScope.Qwen35, "TS_MLX_BATCHED_MOE_DECODE", BoolDialect.DefaultOn,
                "Batched MLX MoE decode (doubles MLX weight memory)."),
            B(nameof(Qwen35Options.MlxMoeFusedGateUpSilu), KnobScope.Qwen35, "TS_MLX_MOE_FUSED_GATE_UP_SILU", BoolDialect.DefaultOn,
                "Fused MLX MoE gate/up/SiLU."),
            B(nameof(Qwen35Options.MlxDeviceRouter), KnobScope.Qwen35, "TS_MLX_DEVICE_ROUTER", BoolDialect.DefaultOn,
                "On-device MLX MoE router."),
            B(nameof(Qwen35Options.MlxEvalDecodeLayerBoundaries), KnobScope.Qwen35, "TS_MLX_EVAL_DECODE_LAYER_BOUNDARIES", BoolDialect.DefaultOff,
                "MLX eval at decode layer boundaries."),
            B(nameof(Qwen35Options.MlxEvalFinalLogits), KnobScope.Qwen35, "TS_MLX_EVAL_FINAL_LOGITS", BoolDialect.DefaultOff,
                "MLX eval of final logits only."),
            B(nameof(Qwen35Options.TpFused), KnobScope.Qwen35, "TS_QWEN35_TP_FUSED", BoolDialect.DefaultOn,
                "Fused tensor-parallel path."),
            B(nameof(Qwen35Options.TpFusedDecode), KnobScope.Qwen35, "TS_QWEN35_TP_FUSED_DECODE", BoolDialect.DefaultOn,
                "Fused TP decode."),
            B(nameof(Qwen35Options.TpFusedPrefill), KnobScope.Qwen35, "TS_QWEN35_TP_FUSED_PREFILL", BoolDialect.DefaultOn,
                "Fused TP prefill."),
            B(nameof(Qwen35Options.TpMoePrefillOnDevice), KnobScope.Qwen35, "TS_TP_MOE_PREFILL_ONDEVICE", BoolDialect.DefaultOn,
                "On-device TP MoE prefill."),
            B(nameof(Qwen35Options.VencFused), KnobScope.Qwen35, "TS_QWEN35_VENC_FUSED", BoolDialect.DefaultOn,
                "Fused vision encoder."),
            B(nameof(Qwen35Options.VencFusedAttn), KnobScope.Qwen35, "TS_QWEN35_VENC_FUSED_ATTN", BoolDialect.DefaultOn,
                "Fused vision-encoder attention."),
            B(nameof(Qwen35Options.VencTrace), KnobScope.Qwen35, "TS_QWEN35_VENC_TRACE", BoolDialect.DefaultOff,
                "Vision-encoder tracing."),
            B(nameof(Qwen35Options.LayerTrace), KnobScope.Qwen35, "TS_QWEN35_LAYER_TRACE", BoolDialect.DefaultOff,
                "Per-layer tracing."),
        };
        ...
        public static string ToMarkdown()
        {
            ...
            sb.AppendLine("construction (KnobResolver). Env changes after a model is created do");
            sb.AppendLine("not affect that model. Bool knobs take `1`/`0`. Unset or empty means");
            sb.AppendLine("the default below; an unrecognized value warns once on stderr and uses");
            sb.AppendLine("the default. Inverted `DISABLE_*` vars disable their feature on `1`;");
            sb.AppendLine("their property stays positive-sense. (The common alternative bool");
            sb.AppendLine("spellings `true`/`false`, `yes`/`no`, `on`/`off` are also accepted,");
            ...
            foreach (var k in All)
            {
                ...
                {
                    BoolDialect.DefaultOn => "default on",
                    BoolDialect.DefaultOff => "default off (opt-in)",
                    BoolDialect.InvertedDisable => "inverted `DISABLE_*` var, default on",
                    _ => "",
                };
